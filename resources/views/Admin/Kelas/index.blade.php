...
@section('content')
   <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Daftar Kelas - {{ $tahunAjaran->nama }}</h1>
          </div>
          <div class="col-sm-6">
                     ...
                     <div class="col-md-12">
                        <div class="card">
                              <div class="card-body card-block">
                                 <form method="post" id="form1" class="form-horizontal" action="{{ route('admin-tahun-ajaran-kelas-store', ['id_tahun_ajaran' => $tahunAjaran->id]) }}" >
                                    @csrf
                                    <input type="hidden" name="tahun_ajaran_id" value="{{ $tahunAjaran->id }}">
                                    <div class="row form-group"{{$errors->has('nama_kelas')?' has-error':''}}>
...
<script>
   $(function () {
      ...
      var table = $('#datatables-kelas').DataTable({
         ...
         processing : true,
         serverSide: true,
         ajax: '{!! route('admin-tahun-ajaran-kelas-data', ['id_tahun_ajaran' => $tahunAjaran->id]) !!}',
         columns: [
